...
def Args():
    ...
    parser.add_argument('--num_frames', type=str, default=None, required=False, help='Number of frames to be generated.')
    parser.add_argument('--use_trans_frame_list', type=str, default=None, required=False, help='Use frame list to process video instead of image.')
    parser.add_argument('--frame_list_dir', type=str, default=None, required=False, help='Frame list directory.')
    ...
    use_trans_frame_list = args.use_trans_frame_list
    frame_list_dir = args.frame_list_dir
    ...
    print({
        ...
        'use_trans_frame_list': use_trans_frame_list,
...
def argument(arg = {}):
    ...
    with open(config_path, 'r') as file:
        ...
        num_frames = config['mode']['num_frames']
        use_trans_frame_list = config['mode']['use_trans_frame_list']
        frame_list_dir = config['mode']['frame_list_dir']
    ...
    if arg['use_api'] != None:
        if arg['use_api'] == "True":
            ...
            use_api = False
    if arg['num_frames'] != None:
        num_frames = int(arg['num_frames'])
    if arg['use_trans_frame_list'] != None:
        if arg['use_trans_frame_list'] == "True":
            use_trans_frame_list = True
        elif arg['use_trans_frame_list'] == "False":
            use_trans_frame_list = False
    if arg['frame_list_dir'] != None:
        frame_list_dir = arg['frame_list_dir']
...
def Preview(config):
    ...
    if emotion_from_tensor:
        ...
        units_ins.step3(input_image_path,emotion_video_path,background_path,background_output_video_path,num_frames,face_landmarker_path,is_trans)
...
def Gen_img_pack(config, output_flie):
    ...
    if emotion_from_tensor:
        units_ins2.step3_pose_from_tensor(input_image_path,emotion_pose_load_path,background_path,output_flie,face_landmarker_path,is_trans,use_trans_frame_list,frame_list_dir)
    else: 
        units_ins2.step3(input_image_path,emotion_video_path,background_path,num_frames,output_flie,face_landmarker_path,is_trans,use_trans_frame_list,frame_list_dir)
    print("----------------------------step3 finished!!-------------------------------------")
...
def process_emotion_video_to_pose_tensor(config):
    ...
    units_preview=units()
    units_preview.process_and_save_emotion_pose(emotion_video_path, background_output_video_path, num_frames, face_landmarker_path, emotion_pose_save_path)
    units_ins=units2()
    units_ins.process_and_save_emotion_pose(emotion_video_path, num_frames, face_landmarker_path, emotion_pose_save_path)
