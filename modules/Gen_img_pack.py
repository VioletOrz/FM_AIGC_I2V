...
class MainFrame2():
    ...
    def update_result_image_bitmap(self,eyebrow_filter,eyes_filter,body_filter,mode,strength,a,use_trans_frame_list):
        if self.biyao==0:
             return
        current_pose = self.pose_converter.convert(self.mediapipe_face_pose)#表示视频当前帧的面部姿势
        #眉毛0~11,使用中等强度的滤波器
        ...
        #眼睛12~25，使用最弱的滤波器，防止眼睛不变
        current_pose[12:26]=eyes_filter.update(current_pose[12:26])
        if use_trans_frame_list: current_pose[0:26] = [0] * 26
         #嘴巴26~36，使用输入的参数控制
        if mode=='a'or mode=='i'or mode=='u'or mode=='e'or mode=='o' or mode=='smirk'or mode=='delta':
            if mode=='a':
                current_pose[26]=strength
        ...
        for i in range(44,45):
          if current_pose[i]<0:
            ...
            if current_pose[i]>=1:
                current_pose[i]=1 
        if use_trans_frame_list: current_pose[37:45] = [0] * 8
        pose = torch.tensor(current_pose, device=self.device, dtype=self.poser.get_dtype())
    ...
    def apply_emotion_pose(self,pose,use_trans_frame_list):
        with torch.no_grad():
            if use_trans_frame_list:
                for i in list(range(0, 26)) + list(range(37, 45)):
...
class units2():
    ...
    def process_and_save_emotion_pose(self,emotion_video_path,num_frames,face_landmarker_path,emotion_pose_save_path):
        ...
        with concurrent.futures.ThreadPoolExecutor() as executor:
            ...
            for mode in transform_mode:
                ...
                if  mode=='a'or mode=='i'or mode=='u'or mode=='e'or mode=='o' :      
                    ...
                    futures.append(executor.submit(self.process_video_to_pose_tensor,emotion_video_path,num_frames,mode,1.0,face_landmarker_path))
                    futures.append(executor.submit(self.process_video_to_pose_tensor,emotion_video_path,num_frames,mode,0.5,face_landmarker_path))
                else:
                    futures.append(executor.submit(self.process_video_to_pose_tensor,emotion_video_path,num_frames,mode,0.0,face_landmarker_path))
    ...
    def process_video_to_pose_tensor(self,emotion_video_path,num_frames,mode,transform_strength,face_landmarker_path):
        parser = argparse.ArgumentParser(description='Control characters with movement captured by iFacialMocap.')
        parser.add_argument(
                '--model',
                type=str,
        ...
        k=1
        pose_tensor_list = []
        while cap.isOpened() and img_num < num_frames:
            img_num+=1
            ret, frame = cap.read()
            if not ret:
                break
    ...
    def process_from_pose_tensor(self,pose_tensor_list,mode,transform_strength,input_image_path,background_path,img_pack_path_name,face_landmarker_path,is_trans=False,use_trans_frame_list=False, frame_list_dir=None):
        img=Image.open(input_image_path)
        image_width,image_height=img.size
        parser = argparse.ArgumentParser(description='Control characters with movement captured by iFacialMocap.')
        parser.add_argument(
        ...
        img_num=0
        if use_trans_frame_list:
            pngs = list_files(frame_list_dir)
            pngs = [f for f in list_files(frame_list_dir) if f.lower().endswith('.png')]

        for pose in pose_tensor_list:
            img_num = img_num + 1
            if use_trans_frame_list:
                numpy_image = main_frame.load_image1(pngs[img_num - 1])
            numpy_image = main_frame.apply_emotion_pose(pose,use_trans_frame_list)
    ...
    def process(self,emotion_video_path,num_frames,mode,transform_strength,input_image_path,background_path,img_pack_path_name,face_landmarker_path,is_trans=False,use_trans_frame_list=False, frame_list_dir=None):
        img=Image.open(input_image_path)
        image_width,image_height=img.size
        parser = argparse.ArgumentParser(description='Control characters with movement captured by iFacialMocap.')
        parser.add_argument(
        ...
        while cap.isOpened() and img_num < num_frames:
            ...
            if use_trans_frame_list:
                if img_num > len(pngs): break
                main_frame.load_image1(pngs[img_num-1])
            ...
            numpy_image=main_frame.update_result_image_bitmap(eyebrow_filter,eyes_filter,body_filter,mode,strength,a,use_trans_frame_list)#嘴巴mode放在这里
            if numpy_image is None:
                continue
    ...
    def step3_pose_from_tensor(self,input_image_path,emotion_pose_load_path,background_path,img_pack_path_name,face_landmarker_path, is_trans=False,use_trans_frame_list=False,frame_list_dir=None):
        transform_mode=['b','a','i','o']
        #transform_mode=['b','a','e','i','o','u']
        ...
        with concurrent.futures.ThreadPoolExecutor() as executor:
            ...
            for mode_strength, pose_tensor_list in emotion_pose_tensor_list.items():
                ...
                if  mode=='a'or mode=='i'or mode=='u'or mode=='e'or mode=='o' :
                    ...
                    #futures.append(executor.submit(self.process, options,emotion_video_path,poser,pose_converter,device,image_width,image_height,mode,input_image_path,background_path))
                    futures.append(executor.submit(self.process_from_pose_tensor,pose_tensor_list,mode,strength,input_image_path,background_path,img_pack_path_name,face_landmarker_path,is_trans,use_trans_frame_list,frame_list_dir))
    ...
    def step3(self,input_image_path,emotion_video_path,background_path,num_frames,img_pack_path_name,face_landmarker_path,is_trans=False,use_trans_frame_list=False,frame_list_dir=None):
        transform_mode=['b','a','i','o']
        #transform_mode=['b','a','e','i','o','u']
        ...
        with concurrent.futures.ThreadPoolExecutor() as executor:
            ...
            for mode in transform_mode:
                ...
                if  mode=='a'or mode=='i'or mode=='u'or mode=='e'or mode=='o' : 
                    ...
                    for s in strength:   
                        #futures.append(executor.submit(self.process, options,emotion_video_path,poser,pose_converter,device,image_width,image_height,mode,input_image_path,background_path))
                        futures.append(executor.submit(self.process,emotion_video_path,num_frames,mode,s,input_image_path,background_path,img_pack_path_name,face_landmarker_path,is_trans,use_trans_frame_list,frame_list_dir))
