...
class units():
    ...
    def step22(self,original_image_path,mask_output_path,background_path):
        #将遮罩图片和原始图片输入sd，重绘得到背景图片
        import requests
        import subprocess
        import io
    ...
    def step3(self,input_image_path,emotion_video_path,background_path,background_output_video_path):
        ...
        out=cv2.VideoWriter(background_output_video_path,fourcc,fps,(background.size[0],background.size[1]))
        ...
        while cap.isOpened() and img_num<100:
            ...
            self.step4(out,background,resized_image)
    ...
    def step4(self,out,background,img): 
...
emotion_video_path = 'tha3/app/emotion2/'+a1#表情视频的路径
...
units_ins.step1(original_image_path,mask_output_dir)
...
a=1
               #这个0和1是用来控制是否启动sd生成背景图片的     
if a==1:
    units_ins.step2(original_image_path,mask_output_dir+'mask_0.png',background_path)
    print("----------------------------step2 finished!!-------------------------------------")
else:
    units_ins.step22(original_image_path,mask_output_dir+'mask_0.png',background_path)
    print("----------------------------step2 finished!!-------------------------------------")
    units_ins.step2(cropped_path,mask_output_dir+'mask_0.png',background_path)
    print("----------------------------step2 finished!!-------------------------------------")
...
units_ins.step3(input_image_path,emotion_video_path,background_path,background_output_video_path)
print("----------------------------step3 finished!!-------------------------------------")


#注意：仅在输入图片分辨率较低的情况下使用，如果输入的图片已经是4k，那么运行这一步电脑可能会炸
#step5:视频超分辨率(background_output->final_output)
# weight_path="D:\\FaceMind_AIGC\\APISR\\pretrained\\2x_APISR_RRDB_GAN_generator.pth"
# units_ins.step5(weight_path,vsr_input_video_path,vsr_output_video_path)
