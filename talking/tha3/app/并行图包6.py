...
class units2():
    ...
    def super_resolve_video(self,weight_path,path):
    # 加载模型权重
    ...
    #path=r"C:\Users\FM\Desktop\other code\input\set1\img_2.webp"

            
        inference.super_resolve_img(generator, path, path, weight_dtype=torch.float32, downsample_threshold=-1, crop_for_4x=False)
    def process(self,emotion_video_path,mode,input_image_path,background_path):
        img=Image.open(input_image_path)
        image_width,image_height=img.size
        parser = argparse.ArgumentParser(description='Control characters with movement captured by iFacialMocap.')
        parser.add_argument(
        ...
        for strength in transform_strength:
                ...
                while cap.isOpened() and img_num<100:
                    ...
                    if not os.path.exists('C:/Users/Violet/Desktop/facial/beishang2/'+mode+'_'+str(strength)):
                        os.makedirs('C:/Users/Violet/Desktop/facial/beishang2/'+mode+'_'+str(strength))
                    #把透明人物拼接到背景上，输出的是pil格式
                    resized_image=self.step4(Image.open(background_path),resized_image,input_image_path)
                    #不超分（二选一）
                ...
                cap.release()
    def process2(self,emotion_video_path,mode,input_image_path,background_path):
        img=Image.open(input_image_path)
        image_width,image_height=img.size
        parser = argparse.ArgumentParser(description='Control characters with movement captured by iFacialMocap.')
        parser.add_argument(
        ...
        #with ProcessPoolExecutor() as executor:
         ...
         for mode in transform_mode:
            ...
            if  mode=='a'or mode=='i'or mode=='u'or mode=='e'or mode=='o' :
                ...
                futures.append(executor.submit(self.process,emotion_video_path,mode,input_image_path,background_path))

              
            else:
                futures.append(executor.submit(self.process2,emotion_video_path,mode,input_image_path,background_path))
    ...
    def step4(self,background,img,input_image_path): 
...
original_image_path='tha3/app/ai-pic/ld_01.png'#原始图片的输入路径
...
emotion_video_path2 = 'tha3/app/emotion2/'+a2#表情视频的路径
units_ins2=units2()
time1=time.time()
...
path1=r"C:/Users/Violet/Desktop/facial/beishang2/o_0.5"
shutil.rmtree(path1)
path2=r"C:/Users/Violet/Desktop/facial/beishang2/i_0.5"
shutil.rmtree(path2)
