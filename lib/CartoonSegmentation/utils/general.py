...
def calculate_center(topleft, bottomright):
    center_x = (topleft[0] + bottomright[0]) // 2
    center_y = (topleft[1] + bottomright[1]) // 2
    return center_x, center_y
def enlarge_bbox(k,topleft, bottomright):
    center = calculate_center(topleft, bottomright)
    new_width = int(1.0*(bottomright[0] - topleft[0]))
    new_height = int(1.0*(bottomright[1] - topleft[1]))
    if k==0:#小头像
        enlarged_topleft = (center[0] - new_width, center[1] - int(1.4*new_height))  # 扩大bbox
        enlarged_bottomright = (center[0] + new_width, center[1] + int(0.6*new_height))  # 扩大bbox
    else:#大头像
        enlarged_topleft = (center[0] - new_width-int(0.5*new_width), center[1] - int(1.4*new_height)-int(0.5*new_width))  # 扩大bbox
        enlarged_bottomright = (center[0] + new_width+int(0.5*new_width), center[1] + int(0.6*new_height)+int(0.5*new_width))  # 扩大bbox
    
    return enlarged_topleft, enlarged_bottomright

def plot_one_box(z,x, img, color=None, label=None, line_thickness=None,save_path=''):
    ...
    c2 = tuple(c2)
    c1 = tuple(c1)
    cropped_image=img[c1[1]:c2[1], c1[0]:c2[0]]       # 裁剪头像
    if z==0:                                         #因为要两次调用这一个函数，但是实际我们只要第一次生成的图片就可以了，因此这边有一个判断
      cv2.imwrite(save_path + "cropped_image_large.png",cropped_image)
      #print(a)
    k=0                                                # 0代表小头像，1代表大头像
    tl = line_thickness or round(0.002 * (img.shape[0] + img.shape[1]) / 2) + 1  # line/font thickness
    color = color or [random.randint(0, 255) for _ in range(3)]
    ...
    print(c1,c2)
    c1, c2 = enlarge_bbox(k,c1, c2)
    a1, a2 = calculate_center(c1, c2) # 获取中心点位置
    c2 = list(c2)
    c1 = list(c1)
    if c1[1]<0:
        c2[1] = c2[1] - c1[1]
    ...
    cropped_image=img[c1[1]:c2[1], c1[0]:c2[0]]       # 裁剪头像
    if z==0:                                         #因为要两次调用这一个函数，但是实际我们只要第一次生成的图片就可以了，因此这边有一个判断
      cv2.imwrite(save_path + "cropped_image_small.png",cropped_image)
