            ...
            if det is not None and len(det):
                ...
                for *xyxy, conf, cls in det:
                    ...
                    if save_img or view_img:  # Add bbox to image
                        ...
                        c1,c2,length=plot_one_box(z,xyxy, im0, color=colors[int(cls)], line_thickness=3)
                        z=z+1
    return c1,c2,length

   
  parser = argparse.ArgumentParser()
  parser.add_argument('--weights', nargs='+', type=str, default='D:\FaceMind_AIGC\CartoonSegmentation\yolov5x_anime.pt', help='model.pt path(s)')
...
def save_masks(img_path, output_dir,cropped_dir):
    # 初始化检测器
    detector = AnimeInsSeg("D:\\FaceMind_AIGC\\CartoonSegmentation\\models\\AnimeInstanceSegmentation\\rtmdetl_e60.ckpt",device='cuda')
    ...
    if not instances.is_empty:
        ...
        for ii, mask in enumerate(instances.masks):
            ...
            c1,c2,length=change_position(image_path)
            #新图像脸的中心位置
            x=int(max_side//2)
            y=int(max_side//4)
...
def crop_image(image_path):
    ...
    img_path=r"D:\FaceMind_AIGC\talking\tha3\app\source_imgs\cropped_image.png"
    # 保存裁剪后的图像
