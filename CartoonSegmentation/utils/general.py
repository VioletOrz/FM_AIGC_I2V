...
def plot_one_box(z,x, img, color=None, label=None, line_thickness=None):
    ...
    if z==0:                                         #因为要两次调用这一个函数，但是实际我们只要第一次生成的图片就可以了，因此这边有一个判断
      cv2.imwrite("cropped_image_large.png",cropped_image)
    k=0                                                # 0代表小头像，1代表大头像
    tl = line_thickness or round(0.002 * (img.shape[0] + img.shape[1]) / 2) + 1  # line/font thickness
    color = color or [random.randint(0, 255) for _ in range(3)]
    c1, c2 = (int(x[0]), int(x[1])), (int(x[2]), int(x[3]))
    ...
    cropped_image=img[c1[1]:c2[1], c1[0]:c2[0]]       # 裁剪头像
    if z==0:                                         #因为要两次调用这一个函数，但是实际我们只要第一次生成的图片就可以了，因此这边有一个判断
      cv2.imwrite("cropped_image_small.png",cropped_image)
